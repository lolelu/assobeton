...
@section('content')
    <div class="form" v-sticky data-sticky-id="navbar" data-sticky-offset="0" data-sticky-topoffset="12">
        ...
        <form action="{{ $saveUrl }}" novalidate method="POST"
            @if ($customForm) ref="customForm" @else v-on:submit.prevent="submitForm" @endif>
...
@section('initialStore')
    ...
    {{ json_encode(($schedule ?? true) && isset($item) && $item->isFillable('publish_start_date')) }},
    ...
    window['{{ config('twill.js_namespace') }}'].STORE.medias.crops = {!! json_encode(
        ($item->mediasParams ?? []) + config('twill.block_editor.crops') + (config('twill.settings.crops') ?? []),
    ) !!}
...
@prepend('extra_js')
    @includeWhen(config('twill.block_editor.inline_blocks_templates', true),
        'twill::partials.form.utils._blocks_templates')
